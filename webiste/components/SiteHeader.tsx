    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#11100e]/75 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
        ...
        <nav className="hidden items-center gap-7 text-sm text-[#d8cec0] md:flex">
          {navItems.map((item) => (
            <a key={item.label} href={item.href} className="transition hover:text-[#d6aa63]">
              {item.label}
            </a>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="/products#cart" className="relative grid size-10 place-items-center rounded border border-white/15">
            <ShoppingBag size={19} />
            {cartCount > 0 && (
